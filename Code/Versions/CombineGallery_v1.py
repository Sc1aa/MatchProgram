...
#   This command tells tkinter: "Place this frame at the very bottom of the
#   available space and make it fill horizontally." Its vertical space is now reserved.
...
class ImageCombinerApp:
    def __init__(self, root):
        ...
        self.controls_frame = Frame(root, bg="#333")
        self.controls_frame.pack(side=BOTTOM, fill=X, pady=2) # Dock to the BOTTOM

        self.info_label = Label(root, text="No images loaded", fg="white", bg="#222", font=("Arial", 10))
        self.info_label.pack(side=BOTTOM, fill=X, pady=3)      # Dock to the BOTTOM

        self.display_frame = Frame(root, bg="#222")
        self.display_frame.pack(expand=True, fill=BOTH)       # Now this fills the REMAINING space
